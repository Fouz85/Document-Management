...
const QATAR_PHONE = /^(\+974|974)?[3567]\d{7}$/;

const RECORD_TYPES = ['Files', 'Registers', 'Maps', 'Engineering Designs', 'Photos'];
        ...
        <div class="card-body p-4">
          <div class="row g-3">
            ...
            <div class="col-md-6">
              ...
              <input id="DestructionNo" class="form-control" formControlName="destructionNo"
                     placeholder="2026\\001" [class.field-invalid]="invalid('destructionNo')">
              @if (invalid('destructionNo')) { <span class="field-error-msg">{{ i18n.t('request.destructionNoHint') }}</span> }
            </div>
          </div>
        </div>
      </div>
          ...
          <div class="row g-3">
            <div class="col-md-6">
              ...
              <select id="UnitSelect" class="form-control"
                      ...
                      [value]="selectedUnitId()" (change)="pickUnit($event)">
                <option value="">-- {{ i18n.t('request.selectDepartment') }} --</option>
                @for (u of rootUnits(); track u.id) { <option [value]="u.id">{{ u.name }}</option> }
              </select>
            </div>
            <div class="col-md-6">
              ...
              <select id="SubSubSelect" class="form-control"
                      [class.readonly-field]="lockedDept()"
                      [style.pointer-events]="lockedDept() ? 'none' : 'auto'"
                      [value]="selectedSectionId()" (change)="pickSection($event)">
                <option value="">-- {{ i18n.t('request.selectSection') }} --</option>
                @for (u of sections(); track u.id) { <option [value]="u.id">{{ u.name }}</option> }
              </select>
            </div>
            <div class="col-md-6">
              <label class="form-label" for="ResponsibleOfficer">{{ i18n.t('request.responsibleOfficer') }} <span class="text-danger">*</span></label>
              <input id="ResponsibleOfficer" class="form-control readonly-field" formControlName="responsibleOfficer" readonly>
            </div>
            <div class="col-md-6">
              <label class="form-label" for="Email">{{ i18n.t('request.email') }} <span class="text-danger">*</span></label>
              <input id="Email" type="email" class="form-control readonly-field" formControlName="email" readonly dir="ltr">
            </div>
            <div class="col-md-6">
              <label class="form-label" for="Phone">{{ i18n.t('request.phone') }} <span class="text-danger">*</span></label>
              <input id="Phone" class="form-control" formControlName="phone" dir="ltr"
                     [placeholder]="i18n.t('request.phonePlaceholder')" [class.field-invalid]="invalid('phone')">
              <div class="form-text text-muted">{{ i18n.t('request.phoneHint') }}</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="card-body p-4">
          ...
          <div class="row g-3">
            <div class="col-12">
              ...
              <select id="StorageLocation" class="form-control" formControlName="storageLocation"
              ...
              <thead>
                <tr class="text-center" style="vertical-align:middle;">
                  ...
                  <th style="min-width:180px;padding:10px 6px;line-height:1.6;">
                    {{ i18n.t('request.recordsTitle') }}<br><small>Records Title/Description</small> <span class="text-danger">*</span>
                  </th>
                  <th style="width:95px;padding:10px 6px;line-height:1.6;">
                    {{ i18n.t('request.originalOrCopy') }}<br><small>Original | Copy</small> <span class="text-danger">*</span>
                  </th>
                  <th style="width:140px;padding:10px 6px;line-height:1.6;">
                    {{ i18n.t('request.recordsType') }}<br><small>Records Type</small> <span class="text-danger">*</span>
                  </th>
                  <th style="width:140px;padding:10px 6px;line-height:1.6;">
                    {{ i18n.t('request.storageMedium') }}<br><small>Records Storage Medium</small> <span class="text-danger">*</span>
                  </th>
                  <th style="width:105px;padding:10px 6px;line-height:1.6;">
                    {{ i18n.t('request.retentionRuleNo') }}<br><small>Optional</small>
                  </th>
                  <th style="width:90px;padding:10px 6px;line-height:1.6;">
                    {{ i18n.t('request.firstYear') }}<br><small>First Date</small> <span class="text-danger">*</span>
                  </th>
                  <th style="width:90px;padding:10px 6px;line-height:1.6;">
                    {{ i18n.t('request.lastYear') }}<br><small>Last Date</small> <span class="text-danger">*</span>
                  </th>
                  <th style="width:80px;padding:10px 6px;line-height:1.6;">
                    {{ i18n.t('request.recordsVolume') }}<br><small>Vol. (m)</small> <span class="text-danger">*</span>
                  </th>
                  <th style="min-width:130px;padding:10px 6px;line-height:1.6;">
                    {{ i18n.t('request.remarks') }}<br><small>Optional</small>
                  </th>
              ...
              <tbody formArrayName="records">
                @for (rec of records.controls; track rec; let i = $index) {
                  <tr [formGroupName]="i">
                    ...
                    <td><input class="form-control form-control-sm" formControlName="recordsTitle"></td>
                    <td>
                      <select class="form-select form-select-sm" formControlName="originalOrCopy">
                        <option value="">—</option>
                    ...
                    <td>
                      <div style="display:flex;gap:5px">
                        <select class="form-select form-select-sm" style="flex:2;"
                                [value]="typeSelect(rec)" (change)="setTypeChoice(rec, $event)">
                          <option value="">—</option>
                    ...
                    <td>
                      <div style="display:flex;gap:5px">
                        <select class="form-select form-select-sm" style="flex:2;"
                                [value]="mediumSelect(rec)" (change)="setMediumChoice(rec, $event)">
                          <option value="">—</option>
                    ...
                    <td><input type="number" step="0.01" min="0" class="form-control form-control-sm" formControlName="recordsVolume" placeholder="0.00"></td>
...
export class RequestFormComponent {
  ...
  private syncDepartment(): void {
    ...
    this.form.patchValue({ department: [unit?.name, section?.name].filter(Boolean).join(' - ') });
  }

  addRecord(): void {
    this.records.push(this.fb.group({
      recordsTitle: [''], originalOrCopy: [''], recordsType: [''], storageMedium: [''],
      retentionRuleNo: [''], firstYear: [null as number | null], lastYear: [null as number | null],
      recordsVolume: [null as number | null], remarks: ['']
    }));
  }
  ...
  private buildDto(asDraft: boolean): SaveDestructionRequestDto {
    ...
    return {
      ...
      records: (v.records as Record<string, unknown>[]).map((r, i) => ({
        ...
        retentionRuleNo: (r['retentionRuleNo'] as string) || null,
